...
func Handle(pat string, f func(*Proc) *Proc) {
	http.HandleFunc(pat, func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := io.Copy(w, p); err != nil {
			w.Header().Add("proc.err", err.Error())
		}
		r.Body.Close()
	})
}
...
func (p *Proc) Post(url, contentType string) *Proc {
	return Fun(func(w io.Writer) error {
		...
		_, err = io.Copy(w, resp.Body)
		if s := resp.Header.Get("proc.err"); s != "" {
			err = errOr(err, errors.New(s))
		}
		return errOr(err, resp.Body.Close())
	})
}
...
func (p *Proc) Send(addr string) *Proc {
	return Fun(func(w io.Writer) error {
		...
		go func() {
			...
			ch2 <- errOr(errors.Unwrap(err), conn.(*net.TCPConn).CloseWrite())
		}()
		select {
		case err = <-ch1:
		case err = <-ch2:
